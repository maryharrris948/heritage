    ...
    <div className="px-[16px]">
      <div className="w-full">
        <div className="text-[14px] w-full font-[500] flex items-center justify-between">
          ...
          <Link href="/dashboard/transactions" className="text-[13px] font-[500] text-[#14509B]">
            See All
          </Link>
        </div>
          ...
          {transactionsToShow.map((transaction: Transaction) => (
            <div key={transaction.transaction_id} className="flex justify-between py-3">
              <div className="flex gap-2 text-gray-800">
                <button className="border-none flex items-center justify-center outline-none rounded-full w-[35px] h-[35px] bg-[#14509B]/10">
              ...
              <div className="flex flex-col gap-1 text-right">
                ...
                <span
                  className={`text-xs font-medium ${
                    transaction.status === 'Pending' ? 'text-yellow-500 font-bold' : transaction.status === 'On Hold' ? 'text-yellow-500 font-bold' : 'text-green-600'
                  }`}
                >
                  {transaction.status}
                </span>
